...
**Plans**: 5 plans
- [ ] 23-01-PLAN.md — docker.yml foundation: job-level permissions (id-token + attestations: write) + `id: build` + cosign-installer + cosign sign (ATTEST-01)
...
- [ ] 23-03-PLAN.md — ci.yml sigstore-pin-check job: 4-action grep gate (sigstore/cosign-installer + actions/attest-build-provenance + actions/attest-sbom + anchore/sbom-action)
- [ ] 23-04-PLAN.md — SECURITY.md § Supply-chain status rewrite: new `### Image signatures + attestations (v1.6 onward)` subsection with cosign verify + gh attestation verify + cosign save recipes (ATTEST-04) + Pitfall 10 + Pitfall 13 callouts
...
| 23. cosign Image Attestations + SLSA + SBOM | v1.6 | 0/5 | Planned     | — |
