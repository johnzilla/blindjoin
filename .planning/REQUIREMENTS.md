...
- [x] **ADVERT-01**: `BipConfig` section in `coordinator.toml` (plus `BLINDJOIN__COORDINATOR__BIP__*` env-var overrides) with `allow_p2wpkh`, `allow_p2tr`, `allow_p2sh_p2wpkh` flags (default all `true`); validated at startup via `CoordinatorConfig::validate()` — fail-fast at boot, never panic-at-first-request
...
| BIP322-04 | Phase 15 | Complete |
| ADVERT-01 | Phase 16 | Complete |
| ADVERT-02 | Phase 16 | Complete |
| ADVERT-03 | Phase 16 | Complete |
