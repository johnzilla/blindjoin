...
- [ ] **TEST-01**: CI installs a pinned `bitcoind` binary (cached between runs) so integration tests can spawn it without per-job download cost
- [ ] **TEST-02**: Integration tests that require bitcoind actually execute in CI on every PR — no silent graceful-skips. The CI test count includes them.
