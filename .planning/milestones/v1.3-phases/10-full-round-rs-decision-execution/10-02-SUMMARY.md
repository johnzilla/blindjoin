...
key-decisions:
  ...
  - "User-authorized Fix A applied to Plan 10-01's sealed mod.rs::fund_regtest (commit d99b3a4): reorder get_raw_transaction_verbose before generate_to_address so the vout reads land while the funding txs are still in the mempool. This unblocked the first failure layer."
  - "User-authorized Fix WIF-D applied to client/src/wallet.rs (commit e02ce55): switch ClientWallet::from_wif from Wallet::create(d, d) to Wallet::create_single(d). bdk_wallet 2.3 rejects identical external+internal descriptors; create_single is bdk's purpose-built API for keychain-less wallets. This was the second blocker uncovered after Fix A unblocked the helper. Scope: production code, strictly outside CONTEXT.md test-infrastructure scope — applied per user authorization at the second checkpoint."
  - "Third blocker uncovered after Fix A + Fix WIF-D: `register_input: Failed to parse coordinator RSA public key: Encoding error` at client/src/round/input.rs:41. The blind-rsa-signatures `BjPublicKey::from_der` rejects the bytes the coordinator emits on /info. This is a coordinator-client protocol-format mismatch — substantially out of Phase 10 scope. Phase 10 execution halted here pending re-planning. D-01's commitment to 'repair all 6 tests' may need to be revisited."
