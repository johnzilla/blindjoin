...
// Phase 18 18-01: helpers promoted to crate root (tests/integration/mod.rs)
// with pub(crate) visibility. `full_round.rs` imports them here so all
// existing callsites compile unchanged (unqualified names). ZERO behaviour
// change — bodies are byte-exact copies now living in mod.rs.
use crate::{v14_p2wpkh_coordinator_info, build_input_reg_round_state, spawn_coordinator, wait_for_coordinator};
